...
rm -rf logs
{{{python tokenizer.py zip >&1 2>&3} | while read line; do printf "\e[32m[tokenizer.py zip] \e[0m$line\n"; done} 3>&1;} | while read line; do printf "\e[32m[{stderr}tokenizer.py zip] \e[0m$line\n"; done}
cat files_tokens/* > blocks.file
cp blocks.file ../../clone-detector/input/dataset/
cd ../../clone-detector
{{{python controller.py 10 >&1 2>&3} | while read line; do printf "\e[32m[controller.py 10] \e[0m$line\n"; done} 3>&1;} | while read line; do printf "\e[32m[{stderr}controller.py 10] \e[0m$line\n"; done}
cd ..
