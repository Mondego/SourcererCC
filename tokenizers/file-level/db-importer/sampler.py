...
c = db.execute('SELECT pjs.projectId, pjs.projectUrl, COUNT(clones.hostId) FROM projects AS pjs JOIN projectClones AS clones ON pjs.projectId = clones.hostId GROUP BY clones.hostId ORDER BY COUNT(clones.hostId) DESC LIMIT %s;' % SAMPLE_SIZE)
...
c = db.execute('SELECT pjs.projectId, pjs.projectUrl, COUNT(clones.cloneId) FROM projects AS pjs JOIN projectClones AS clones ON pjs.projectId = clones.cloneId GROUP BY clones.cloneId ORDER BY COUNT(clones.cloneId) DESC LIMIT %s;' % SAMPLE_SIZE)
...
c = db.execute('SELECT projs.projectUrl,projs2.projectUrl,clones.cloneCloningPercent,clones.cloneClonedFiles,clones.hostAffectedPercent,clones.hostAffectedFiles FROM projects as projs JOIN projectClones as clones ON clones.cloneId = projs.projectId JOIN projects as projs2 ON clones.hostId = projs2.projectId AND clones.cloneCloningPercent > 80 ORDER BY RAND() LIMIT %s;' % SAMPLE_SIZE)
with open('sample-project-pairs.csv','w') as file:
    file.write('projectUrl,projectUrl,cloneCloningPercent,cloneClonedFiles,hostAffectedPercent,hostAffectedFiles\n')
    for (projectUrl,projectUrl,cloneCloningPercent,cloneClonedFiles,hostAffectedPercent,hostAffectedFiles, ) in c:
        file.write("""%s,%s,%s,%s,%s,%s\n""" % (projectUrl,projectUrl,cloneCloningPercent,cloneClonedFiles,hostAffectedPercent,hostAffectedFiles))


logging.info('Sampling non-cloning projects')
c = db.execute('SELECT projectId, projectUrl FROM projects WHERE projectId IN (SELECT * FROM ((SELECT cloneId AS name FROM projectClones) UNION ALL (SELECT hostId AS name FROM projectClones)) AS a) ORDER BY RAND() LIMIT %s;' % SAMPLE_SIZE)
...
c2 = dc.execute('SELECT fls.fileId, fls.relativeUrl FROM files AS fls JOIN stats AS sts ON fls.fileHash=sts.fileHash AND sts.tokenHash IN (SELECT tokenHash FROM stats GROUP BY tokenHash ORDER BY COUNT(tokenHash) DESC) LIMIT %s;' % SAMPLE_SIZE)
counts = []
for (tokenHash,count, ) in c1:
    counts = counts + [count]
with open('top-hashToken-groups.csv','w') as file:
    file.write('fileId,fileUrl\n')
    for (fileId, fileUrl, ) in c:
