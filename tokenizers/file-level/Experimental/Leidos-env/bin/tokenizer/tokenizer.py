...
def read_config():
    global N_PROCESSES, PROJECTS_BATCH, FILE_projects_list
    global PATH_tokens_file_folder, PATH_logs
...
def process_file_contents(proj_id, file_string, container_path, file_path, file_bytes, proj_url, FILE_tokens_file, db):
    ...
    file_hash = get_file_hash(file_string)
...
def start_child(processes, global_queue, proj_paths, batch):
    ...
    p = Process(name='Process '+str(pid), target=process_projects, args=(pid, paths_batch, global_queue, ))
    processes[pid][0] = p
    p.start()
...
def kill_child(processes, pid, n_files_processed):
    ...
    logging.info("Process %s finished, %s files processed (%s). Current total: %s" % (pid, n_files_processed, processes[pid][1], file_count))
...
if __name__ == '__main__':
    ...
    try:
        ...
        print 'Error connecting to MySQL with credentials \''+DB_user+':'+DB_pass+'\'@\'localhost\''
        print e
        sys.exit(1)
    ...
    print 'Project ids start at %d' % (proj_id)
    ...
    proj_paths = zip(range(proj_id, proj_id+len(proj_paths)+1), proj_paths)

    if os.path.exists(PATH_tokens_file_folder) or os.path.exists(PATH_logs):
        print 'ERROR - Folder ['+PATH_tokens_file_folder+'] or ['+PATH_logs+'] already exists!'
        sys.exit()
    else:
