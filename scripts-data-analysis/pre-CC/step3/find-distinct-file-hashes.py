...
for filename in os.listdir(sys.argv[1]):
  if filename.endswith('.stats'):
    ...
    with open(filename, 'r') as csv_file_book, open(os.path.join(PATH_distinct_file_hashes,'distinct-files.stats'),'a+') as result_stats:
...
for filename in os.listdir(sys.argv[2]):
  if filename.endswith('.tokens'):
    ...
    print 'Searching on ', filename
    with open(filename, 'r') as file_book, open(os.path.join(PATH_distinct_file_hashes,'distinct-files.tokens'),'a+') as result_tokens:
