...
2. [How to use SourcererCC](#how-to-use-sourcerercc)
    ...
    2. [Indexing](#step-2-indexing)
    3. [Clone Detection](#step-3-clone-detection)
...
## What is SourcererCC?
SourcererCC is Sourcerer's token-based code clone detector for very large code bases and Internet-scale project repositories. SourcererCC works at many levels of granularity, from block level to project level, depending on specific clone deteciton needs.
...
First, the source code is tokenized in way special way, producing one or more token files. These token files are then fed to an indexer in order to produce an inverted index. Finally, a pair-wise similarity search is performed using the inverted index in order to identify the code clones. We explain each of these steps below; but before that, here are the instructions for how to install and configure SourcererCC.
