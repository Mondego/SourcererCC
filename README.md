...
3. [Build you own parser](#build-your-own-parser)
    1. [Files Generated by the Parser](#files-generated-by-the-parser)
    2. [Clone Granularity](#clone-granularity)
    3. [Example: Parsing a java method into SourcereCC understandable format](#example-parsing-a-java-method-into-sourcerecc-understandable-format)
...
1. Parsed output file. This file is usually named as blocks.file, however, there is no such restriction on the file name. A user can give it any name they want to. However, for the sake of ease we will call it blocks.file. In the blocks.file, each code-snippet is represented in a newline. There is no limit to the number of lines in the blocks.file. Also, there is no limit to the number of such blocks.file files. You can either have one file with all of the parsed code-snippets or multiple files with any number of parsed code-snippets. Just make sure that you put all of these generated files into the *DATASET_DIR_PATH* directory. SourcererCC will index all files present inside this directory. 
