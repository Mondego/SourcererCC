...
##### Files Generated by the Parser
A parser should generate at least two files:

1. Parsed output file. This file is usually named as blocks.file, however, there is no such restriction on the file name. A user can give it any name they want to. However, for the ease we will call it blocks.file. In the blocks.file, each code-snippet is represented in a newline. 
2. Bookkeeping file. This file contains the mapping of code-snippet ids to their file system path. Like blocks.file, a user can name it whatever they want to. In this document this file is referenced as headers.file. Please find more info about this file under the section "Tracking code-snippets from blockIds".
