...
public class CandidateProcessor implements IListener, Runnable {
    ...
    private void processResultWithFilter() throws InterruptedException,
        ...
        for (Entry<Long, CandidateSimInfo> entry : codeBlockIds.entrySet()) {
            ...
            Document doc = null;
            try {
                doc = SearchManager.invertedIndexsearcher.get(shardPath)
                        .getDocument(entry.getKey());
                    ...
                    if (blocks.size() == 1) {
                        ...
                        if (newCt != -1) {
                            ...
                            candidatePair = new CandidatePair(queryBlock,
                                    ...
                                    candidateSize, functionIdCandidate,
                                    candidateId);
                        }
                ...
                logger.error(SearchManager.NODE_PREFIX + e.getMessage()
                        + ", cant parse id for " + doc.get("functionId") + ", "
                        + doc.get("id"));
            } catch (IOException e) {
                logger.error(
                        e.getMessage() + ", can't find document from searcher"
                                + entry.getKey());
            }
        }
    }
