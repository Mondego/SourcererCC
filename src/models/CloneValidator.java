	...
	public void run() {
		try {
			...
			synchronized (SearchManager.lock) {
				//System.out.println("vcq size: "+ SearchManager.verifyCandidateQueue.size() + Util.debug_thread());
				if(SearchManager.verifyCandidateQueue.size()>0){
				candidatePair = SearchManager.verifyCandidateQueue
							.remove();
